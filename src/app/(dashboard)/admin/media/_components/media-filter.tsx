...
export const MediaFilter = () => {
    ...
    const fromDateObj = useMemo(() => (fromDate ? dayjs(fromDate).toDate() : new Date()), [fromDate]);
    const toDateObj = useMemo(() => (toDate ? dayjs(toDate).toDate() : new Date()), [toDate]);
    const fromDateLabel = useMemo(() => (fromDate ? dayjs(fromDate).format('DD MMM YYYY') : 'From Date'), [fromDate]);
    const toDateLabel = useMemo(() => (toDate ? dayjs(toDate).format('DD MMM YYYY') : 'To Date'), [toDate]);
