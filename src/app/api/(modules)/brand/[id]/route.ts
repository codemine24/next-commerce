...
export const PATCH = catchAsync(
  async (
    ...
    // Step 1: Extract id from route params
    const id = (await params).id;

    // Step 2: Parse request body
    const body = await request.json();

    // Step 3: Validate request body against update brand schema
    await payloadValidator(BrandSchemas.updateBrand, body);

    // Step 4: Call service layer to update brand in database
    const result = await BrandServices.updateBrand(id, body);

    // Step 5: Return success response with updated brand
    return successResponse({
      statusCode: httpStatus.OK,
